function CustomerConnect($q, $http) {
    // Public - Configuration
    var CustomerConnect = this;

    this.Config = {
        AccountKey: "KONACA",
        URL: "https://api.mydrycleaner.com/q"
    };
    ...
    this.StartSession = function () {
        ...
        $http
        .get('https://konacleaners.com/connect/api/getSession.php')
        .then(function (data) {
            console.log(data);
            CustomerConnect.Config.Connected = true;
            CustomerConnect.Config.SessionId = data.data.ReturnObject.SessionID;
            CustomerConnect.Config.AccountKey = 'KONACA';
            CustomerConnect.Config.URL = 'https://api.mydrycleaner.com/q';
            deferred.resolve(data.data);
        }, function (data) {
            CustomerConnect.Config.Error = data.data;
    ...
    this.Request = {
        ...
        CreateRequest: function (requestType, body) {
            ...
            $http.post(CustomerConnect.Config.URL, rq).then(function (data) {
                if (requestType == 'Login' && !data.data.Failed) {
                    CustomerConnect.Config.SessionId = data.data.ReturnObject.SessionID;
                    CustomerConnect.Config.CustomerName = data.data.ReturnObject.CustomerName;
                }
                if (requestType == 'Logoff' && !data.data.Failed) {
    ...
    this.User = {
        ...
        SendMessage: function (subject, body, invoiceid) {
            return new CustomerConnect.Request.CreateRequest('MessageToManagerNoUser', new this.MessageToManager(subject, body, invoiceid));
        },
